...
public class TalkEntity: NSManagedObject {
    ...
    convenience init(from talk: Talk, context: NSManagedObjectContext) {
        self.init(context: context)
        self.id = talk.id
        self.title = talk.title
        self.desc = talk.description
        self.speaker = talk.speaker
        self.series = talk.series
        self.biblePassage = talk.biblePassage
        self.dateRecorded = talk.dateRecorded
        self.duration = Int32(talk.duration)
        self.audioURL = talk.audioURL
        self.videoURL = talk.videoURL
        self.imageURL = talk.imageURL
        self.conferenceImageURL = talk.conferenceImageURL
        self.defaultImageURL = talk.defaultImageURL
        self.fileSize = Int64(talk.fileSize ?? 0)
        self.createdAt = Date()
        self.updatedAt = Date()
    }
    ...
    var mediaURLs: [String] {
        [audioURL, videoURL].compactMap { $0 }.filter { !$0.isEmpty }
    }
    
    var hasLocalContent: Bool {
        localAudioURL != nil || localVideoURL != nil
    }
    
    var primaryMediaURL: String? {
        if let localAudio = localAudioURL, !localAudio.isEmpty {
            return localAudio
        }
        if let localVideo = localVideoURL, !localVideo.isEmpty {
            return localVideo
        }
...
extension TalkEntity {
    ...
    // MARK: - Images (simplified to single artwork URL with fallback logic)
...
public class BookmarkEntity: NSManagedObject {
    ...
    convenience init(position: Double, title: String, talk: TalkEntity, context: NSManagedObjectContext) {
        self.init(context: context)
        self.id = UUID()
...
public class ChapterEntity: NSManagedObject {
    ...
    convenience init(chapterTitle: String, startTime: Double, endTime: Double, talk: TalkEntity, context: NSManagedObjectContext) {
        self.init(context: context)
...
public class DownloadTaskEntity: NSManagedObject {
    ...
    convenience init(talkID: String, downloadURL: String, context: NSManagedObjectContext) {
        self.init(context: context)
        self.id = UUID().uuidString
        self.talkID = talkID
    ...
    var downloadStatus: DownloadStatus {
        get {
            DownloadStatus(rawValue: status ?? "") ?? .pending
        }
        set {
...
// DownloadStatus enum is defined in DownloadService.swift
